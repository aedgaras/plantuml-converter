...
Remiantis gautais rezultatais galima formuluoti tris pagrindines eksperimento isvadas.

Pirma, baziniame strukturiniame lygmenyje transformatorius veikia labai tiksliai. Tai patvirtina 100% lauku tikslumas (`268/268`), 100% API keliu tikslumas (`19/19`), 100% HTTP operaciju tikslumas (`27/27`) ir 100% automatiniu testu sekmingumas (`40/40`).

Antra, semantinio atitikimo požiuriu transformatorius teisingai interpretuoja pagrindinius UML elementus ir ju prasme. Tai apima duomenu tipus, paveldejima, kompozicija, asociacijas, kardinalumus ir stereotipus, naudojamus API operaciju modeliavimui.

Trecia, projekto tikslumas yra aukstas tik tuose sluoksniuose, kuriems realizuota aiskia transformavimo logika. Pazangesniu OpenAPI validavimo ir dokumentavimo elementu lygmenyje projektas islieka ribotas. Todėl bendra akademine isvada turetu buti tokia: si sistema yra efektyvi ir patikima bazines strukturines `PlantUML` i `OpenAPI` transformacijos uzdaviniui, taciau jos taikymas yra ribotas, kai reikia issamaus validavimo, sudetingu kompoziciniu schemu arba pilno dokumentacijos metaduomenu islaikymo.
